...
export default function Error({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  useEffect(() => {
    const key = 'prysmor_page_err_reloaded';
  ...
  return (
    <div style={{
      background: '#080808', color: 'white', fontFamily: 'system-ui, sans-serif',
      ...
      <div style={{ textAlign: 'center', padding: '40px', maxWidth: '480px' }}>
        ...
        <div style={{ display: 'flex', gap: '10px', justifyContent: 'center' }}>
          <button
            onClick={() => { sessionStorage.removeItem('prysmor_page_err_reloaded'); reset(); }}
            style={{
