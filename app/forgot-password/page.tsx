...
export default function ForgotPasswordPage() {
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      setStatus("sent");
      setMessage(data.message || "If an active account exists, we sent a password link.");
    } catch (err) {
      setStatus("error");
    ...
    <main className="relative flex min-h-screen items-center justify-center overflow-hidden bg-[#080808] px-5 py-16 text-white">
      ...
      <div className="relative w-full max-w-[420px]">
        <div className="mb-8 flex flex-col items-center text-center">
          ...
          />
          <h1 className="text-2xl font-semibold tracking-tight">Set or reset password</h1>
          <p className="mt-2 text-sm text-white/45">
            For active buyers only. We&apos;ll email a secure link to choose a password.
          </p>
        </div>
        ...
        <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-6 sm:p-8">
          ...
          <p className="mt-5 text-center text-[12px] text-white/40">
            <Link href="/sign-in" className="text-[#39FF6A] hover:underline">
              Back to sign in
            </Link>
          </p>
        </div>
