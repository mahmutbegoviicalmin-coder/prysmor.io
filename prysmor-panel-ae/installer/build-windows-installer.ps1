...
$nsiScript = "$PSScriptRoot\windows\installer.nsi"
$distDir   = "$repoRoot\dist"

Write-Host "╔══════════════════════════════════════════════════════╗"
Write-Host "║  Prysmor AE Windows Installer Builder               ║"
Write-Host "╚══════════════════════════════════════════════════════╝"
Write-Host ""
...
$output = Get-Item "$distDir\PrysmorAE-Setup.exe" -ErrorAction SilentlyContinue
if (-not $output) {
    $output = Get-ChildItem "$distDir\PrysmorAE*.exe" | Sort-Object LastWriteTime | Select-Object -Last 1
}
Write-Host ""
Write-Host "✅  Done!"
