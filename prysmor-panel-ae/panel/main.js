...
// LocalStorage keys
const LS_TOKEN          = 'prysmor_ae_token';
const LS_USER_ID        = 'prysmor_ae_user_id';
...
// ─── Auto-Select State ────────────────────────────────────────────────────────
// Polls After Effects every 500 ms and reloads the clip when the selection changes.
...
function initCS() {
  try {
    ...
    cs = {
      ...
      getHostEnvironment:      function ()       { return { appName: 'AEFT', appVersion: '0.0' }; },
...
async function startLogin() {
  ...
  try {
    ...
    var appName    = hostEnv.appName    || 'After Effects';
...
function enterPanel() {
  ...
  // Try to auto-load whatever is selected in After Effects right now
  refreshClip(true);
...
function refreshClip(silent) {
  ...
  cs.evalScript('getSelectionInfo()', function (raw) {
    ...
    if (!parsed || parsed.error) {
      ...
      showClipEmpty();
      if (!silent) {
        showToast(parsed ? parsed.error : 'Could not read After Effects selection', 'error');
      }
      return;
    }
...
// ─── Download & Insert into After Effects ─────────────────────────────────────
...
async function downloadAndInsert(outputUrl, startTimeSec, replaceMode, clipDurSec) {
  ...
  // ── Try to write to disk and insert into After Effects ────────────────────
  ...
  // Post-process: scale/trim video for comp insert (no separate audio track in AE).
  {
    var ts = Date.now();
    ...
    var postDone = await new Promise(function (resolve) {
      try {
        ...
        if (needsScale) {
          console.log('[Prysmor:postprocess] scale to 1920x1080');
          args.push('-vf', 'scale=1920:1080', '-c:v', 'libx264', '-crf', '16', '-preset', 'fast', '-c:a', 'aac');
        } else {
          console.log('[Prysmor:postprocess] stream copy');
          args.push('-c', 'copy');
        }
    ...
    if (postDone) {
      finalPath = postDone;
      console.log('[Prysmor:postprocess] video ready:', finalPath);
    } else {
      console.warn('[Prysmor:postprocess] video ffmpeg failed — using raw output');
    }

    state.mf.silentAudioPath = null;
  }
  ...
  // Do NOT delete finalPath immediately — After Effects needs time to import it.
  ...
  if (finalPath !== outPath) {
    ...
    }, 30000); // wait 30s so After Effects has finished reading
  }
}
...
async function addToTimeline() {
  ...
  await new Promise(function (resolve) {
    ...
    if (replaceMode) {
      ...
      fn = 'insertClipOnV2("' + esc + '", ' + startTimeSec + ')';
    }
...
function readFileBase64(absPath) {
  return new Promise(function (resolve, reject) {
    if (!window.cep || !window.cep.fs) {
      return reject(new Error('cep.fs not available — run inside After Effects'));
    }
...
var VERSION_API = 'https://prysmor-io.vercel.app/api/panel/version/ae';
 ...
 * to restart After Effects to apply the update.
 */
function showUpdateBanner(version) {
  try {
    ...
    console.log('[Prysmor:update] Updated to', version, '— please restart After Effects.');
  }
}
